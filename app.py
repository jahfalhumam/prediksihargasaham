...
if st.sidebar.button("🏠"):
    st.session_state["halaman"] = "beranda"
    st.session_state["mulai"] = False
data_terbaru = st.sidebar.checkbox("**Data Terbaru***", value=True)
st.sidebar.markdown(
    '<p style="font-size:12px; margin-top: -15px;"><i><b>*</b>Jika tidak dicentang, maka akan ditampilkan hasil pada saat penelitian.</i></p>',
    unsafe_allow_html=True
)
kode = st.sidebar.radio("**Pilih Kode Saham**", options=["BRPT", "BUMI", "EMTK"], index=0)
kode_jk = kode + ".JK"
if data_terbaru:
    data_download = yf.download(kode_jk, start='2014-05-06', auto_adjust=False)
...
if kode == "BRPT":
    ...
    jumlah_epoch = 100
    window_size = 14
    file_model = "model_brpt.keras"
    riwayat = np.load("riwayat_brpt.npz")
if kode == "BUMI":
    ...
    jumlah_epoch = 75
    window_size = 14
    file_model = "model_bumi.keras"
    riwayat = np.load("riwayat_bumi.npz")
if kode == "EMTK":
    ...
    jumlah_epoch = 100
    window_size = 14
    file_model = "model_emtk.keras"
    riwayat = np.load("riwayat_emtk.npz")
...
if halaman == "beranda":
    ...
    import pandas as pd
    jumlah_download = int(len(data_download))
    tanggal_download_first = data_download.index[0].strftime('%d-%m-%y')
    tanggal_download_last = data_download.index[-1].strftime('%d-%m-%y')
    ...
    jumlah_interpolate = int(len(data_interpolate))
    x = data_interpolate.drop(columns=['Close']).values
    y = data_interpolate['Close'].values  
    jumlah_uji = jumlah_interpolate - jumlah_latih
    data_latih_x, data_uji_x = np.split(x, [jumlah_latih])
    data_latih_y, data_uji_y = np.split(y, [jumlah_latih])
    from sklearn.preprocessing import MinMaxScaler
    scaler_x = MinMaxScaler()
    scaler_y = MinMaxScaler()
    data_latih_x_normal = scaler_x.fit_transform(data_latih_x)
    data_uji_x_normal = scaler_x.transform(data_uji_x)
    data_latih_y_normal = scaler_y.fit_transform(data_latih_y.reshape(jumlah_latih, 1))
    data_uji_y_normal = scaler_y.transform(data_uji_y.reshape(jumlah_uji, 1))
    forecast_horizon = 1
    latih_x = np.array([data_latih_x_normal[i - window_size:i] for i in range(window_size, len(data_latih_x_normal) - forecast_horizon + 1)])
    uji_x = np.array([data_uji_x_normal[i - window_size:i] for i in range(window_size, len(data_uji_x_normal) - forecast_horizon + 1)])
    latih_y = np.array([data_latih_y_normal[i + forecast_horizon - 1] for i in range(window_size, len(data_latih_y_normal) - forecast_horizon + 1)])
    uji_y = np.array([data_uji_y_normal[i + forecast_horizon - 1] for i in range(window_size, len(data_uji_y_normal) - forecast_horizon + 1)])
    from tensorflow.keras.models import load_model
    model = load_model(file_model, compile=False)
    model.compile(optimizer='adam', loss='mse', metrics=['mse'])
    tanggal_prediksi = pd.date_range((data_interpolate.index)[-1] + pd.Timedelta(days=1), periods=jumlah_future)
    prediksi = model.predict(uji_x[-jumlah_future:])
    prediksi_denormal = scaler_y.inverse_transform(prediksi)
    data_prediksi = pd.DataFrame(prediksi_denormal, columns=['Prediksi'])
    if len(tanggal_prediksi) == len(data_prediksi):
        data_prediksi['Tanggal'] = tanggal_prediksi
        data_prediksi.set_index('Tanggal', inplace=True)
        data_ffill = data_download_relevan.reindex(tanggal_full, method='ffill')
        ...
        with st.expander("**📊 Hasil Prediksi**", expanded=True):
            ...
            data_historis_display = jumlah_interpolate - 730
            import plotly.graph_objects as go
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=data_ffill.index[data_historis_display:], 
                y=data_ffill['Close'][data_historis_display:], 
                mode='lines', 
                name='Data Historis'
            ))
            fig.add_trace(go.Scatter(
        ...
        with st.expander("**📊 Data Historis**"):
            ...
            with col1:
                st.metric(label=":blue[Data Historis]", value=jumlah_download)
            with col2:
                st.metric(label=":blue[Tanggal Pertama]", value=tanggal_download_first)
            with col3:
                st.metric(label=":blue[Tanggal Terakhir]", value=tanggal_download_last)
            with col4:
                st.metric(label=":blue[Kode Saham]", value=kode)
            st.write(" ")
            st.write("**Tabel Data Historis:**")
            st.dataframe(data_download)
            st.write(" ")
            st.write("**Grafik Data Historis:**")
            fig = go.Figure()
            for kolom in data_download.columns:
                if kolom != 'Volume':
                    fig.add_trace(go.Scatter(
                        x=data_download.index, 
                        y=data_download[kolom], 
                        mode='lines', 
                        name=kolom
                    ))
            fig.update_layout(
        ...
        with st.expander("**📊 Preprocessing Data**"):
                ...
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric(label=":blue[Data Preprocessed]", value=jumlah_interpolate)
                with col2:
                    st.metric(label=":blue[Data Latih]", value=jumlah_latih)
                with col3:
                    st.metric(label=":blue[Data Uji]", value=jumlah_uji)
                with col4:
                ...
                st.write(" ")
                st.write("**Grafik Perbandingan Data Latih & Uji (y):**")
                fig = go.Figure()
                fig.add_trace(go.Scatter(
                    x=data_interpolate.index[:jumlah_latih], 
                    y=data_interpolate['Close'][:jumlah_latih], 
                    mode='lines', 
                    name='Data Latih'
                ))
                fig.add_trace(go.Scatter(
                ...
                fig.update_layout(
                    xaxis_title='Waktu',
                    yaxis_title='Harga (Rp)',
                    hovermode='x',  # Biar hover muncul detail per titik sumbu X
                    legend_title='Keterangan',
                    margin=dict(t=20, b=50)
                )
                st.plotly_chart(fig)
                ...
                st.write(" ")
                st.write("**Tabel Informasi Hyperparameter:**")
                data_hyperparameter = {
                    "Hyperparameter": ["Window Size", "Forecast Horizon"],
                    "Ukuran": [window_size, forecast_horizon]
                }
                st.dataframe(data_hyperparameter)
        ...
        with st.expander("**📊 Evaluasi Model**"):
                ...
                uji_y_denormal = scaler_y.inverse_transform(uji_y[-jumlah_future:])
                data_ke = list(range(1, jumlah_future + 1))
                uji_y_denormal_list = uji_y_denormal.flatten().tolist()
                prediksi_denormal_list = prediksi_denormal.flatten().tolist()
                dataY = {
                    "Data ke-": data_ke,
                ...
                mse = mean_squared_error(uji_y_denormal, prediksi_denormal)
                rmse = np.sqrt(mse)
                mae = mean_absolute_error(uji_y_denormal, prediksi_denormal)
                mape = mean_absolute_percentage_error(uji_y_denormal, prediksi_denormal) * 100
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                ...
                st.write(" ")
                st.write("**Grafik Akurasi Data Uji Aktual dan Prediksi:**")
                fig = go.Figure()
                fig.add_trace(go.Scatter(
                    x=dataY['Data ke-'],
                    y=dataY['Aktual'],
                ...
                st.plotly_chart(fig)
                st.write(" ")
                st.write("**Tabel Akurasi Data Uji Aktual dan Prediksi:**")
                st.dataframe(dataY)
